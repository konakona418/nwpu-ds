...
void mat_fetch_add(matrix_t* mat, int row, int col, MatrixElemType val) {
    MatrixElemType* ptr = mat_at(mat, row, col);
    if (ptr == NULL) {
        VecElemType triplet = {row, col, val};
        push_back(mat->data, triplet);
...
matrix_t* mat_mul(matrix_t* mat1, matrix_t* mat2) {
    if (mat1->col != mat2->row) {
        return NULL;
    }
    matrix_t* mat = create_matrix(mat1->row, mat2->col);
    for (int i = 0; i < mat1->data->size; i++) {
        for (int j = 0; j < mat2->data->size; j++) {
            VecElemType* triplet1 = at_ref(mat1->data, i);
            VecElemType* triplet2 = at_ref(mat2->data, j);
            if (triplet1->col == triplet2->row) {
                mat_fetch_add(mat, triplet1->row, triplet2->col, triplet1->val * triplet2->val);
            }
        }
    }
    return mat;
...
int main() {
    ...
    matrix_t* mul = mat_mul(mat1, mat2);
    sort_mat(mul);
    print_mat(mul);
